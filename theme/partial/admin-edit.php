...
<body id="razor-admin" class="ng-cloak razor-admin" ng-controller="edit" ng-init="init()">
	...
	<?php if ($this->logged_in > 0): ?>
		<div class="razor-access-panel">
			<i class="razor-logo razor-logo-50 razor-logo-black-circle dashboard-icon mobile-hide-inline-block" ng-click="persist = !persist"></i>
			<i class="razor-logo razor-logo-25 razor-logo-black-circle dashboard-icon mobile-show-inline-block" ng-click="persist = !persist"></i>
			<div class="account-details text-right" ng-class="{'persist': persist}" ng-show="user.id">
				<?php if ($this->logged_in > 5): ?>
					<i class="fa fa-cog fa-4x pull-left"></i>
				<?php endif ?>
				<span class="name">
					{{user.name}} 
